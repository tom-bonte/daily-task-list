...
- A single task is copied to the next day from the edit sheet's "→ Tomorrow" button (it names the date when the day on screen is not today) or with `C` on the selected task; `copyTaskToDay` in `app.js` leaves the original where it is, skips a target day that already has a task with the same `normText` (as "Plan tomorrow" does), and offers "Open" through `ask()`. The button copies what is on screen, including edits not yet saved.
