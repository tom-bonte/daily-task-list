...
- Keyboard shortcuts (in the `keydown` handler): `1`–`5` switch views, `N` focuses the add field, `/` opens Search, `←`/`→` change day, `T` jumps to today, `J`/`K` select a task, `Space` starts/stops its timer, `E` edits it, `X` completes it, `C` copies it to the next day. Modifier combos are ignored, so ⌘C still copies text.
...
- Days are keyed by **local** date. "Plan tomorrow" and "Start from <last day>" use `carryOver`, which copies **only** tasks marked "repeat daily" (user preference), with fresh ids and zero time. Both build on `copyForDay`, which is also what the per-task copy uses: the plan (text, category, planned minutes, repeat) carries over and the logged time does not, so a copy of a running task is not running.
- A single task is copied to the next day three ways: the `.rowcopy` arrow on the task row (hidden until the row is hovered or focused, and hidden altogether below 760px, where there is no hover and the row is tight), the edit sheet's "→ Tomorrow" button (both name the date when the day on screen is not today, via `nextLabel`), or `C` on the selected task; `copyTaskToDay` in `app.js` leaves the original where it is, skips a target day that already has a task with the same `normText` (as "Plan tomorrow" does), and offers "Open" through `ask()`. The button copies what is on screen, including edits not yet saved.
...
- The helper talks to Firebase itself (`menubar/Backend.swift`): Google OAuth with PKCE on a loopback port, traded for a Firebase session via `signInWithIdp`; only the refresh token is kept, in the Keychain. It reads `users/{uid}/meta/settings` for the running timers and stops one by closing its open session in the day document, mirroring `stopRunning`/`tidySessions`. That works on any Space and with the app closed, unlike the accessibility route, which macOS only serves while the app's window is on the current Space. The OAuth client id and secret live in `~/Library/Application Support/HabitsRabbits/oauth.json` (never in the repo); without that file the helper falls back to reading the published state through accessibility.
